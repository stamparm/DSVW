...
NAME, VERSION, GITHUB, AUTHOR, LICENSE = "Damn Small Vulnerable Web (DSVW) < 100 LoC (Lines of Code)", "0.1b", "https://github.com/stamparm/dsvw", "Miroslav Stampar (@stamparm)", "Public domain (FREE)"
    ...
    def do_GET(self):
        ...
        try:
            if path == '/':
                ...
                elif "comment" in params or query == "comments":
                    if "comment" in params:
                        cursor.execute("INSERT INTO comments VALUES(NULL, '%s')" % params["comment"])
                        content += "Thank you for leaving the comment. Please click here <a href=\"/?comments\">here</a> to see all comments%s" % HTML_POSTFIX
                    else:
                        cursor.execute("SELECT id, comment FROM comments")
                        content += "<div><span>Comment(s):</span></div><table><thead><th>id</th><th>comment</th></thead>"
                        content += "".join("<tr>%s</tr>" % "".join("<td>%s</td>" % _ for _ in row) for row in cursor.fetchall())
                        content += "</table>%s" % HTML_POSTFIX
